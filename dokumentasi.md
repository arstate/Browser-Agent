...
155. **Eliminasi Total Duplikasi Multi-Agent, Pembersihan Berkas Numerik & Pengawal Dedup 4 Lapis (`v2.150.272`):**
    - **Akar Masalah**:
      - Pengguna melaporkan agen `ARYA-MAGANG-KOMINFO` dan beberapa agen Djadi Creative muncul duplikat di menu pilihan agen dan dropdown `@mention` (`kok ada yang duplikat ya bro`).
      - Penyebabnya: berkas numerik lama (`14.md`, `40.md`, `67.md`, `72.md`, `73.md`) berada di direktori yang sama dengan berkas alias semantik baru (`arya_magang_kominfo.md`, `djadi_master_orchestrator.md`, dll). Karena ID-nya berbeda (`14` vs `arya_magang_kominfo`), fungsi `list_agents` membaca kedua berkas dan menyajikannya sebagai dua agen terpisah dengan nama yang sama.
      - Selain itu, logika deduplikasi di frontend (`options.js` dan `sidepanel.js`) hanya memeriksa kesamaan ID (`item.id === ag.id`), sehingga dua agen dengan ID berbeda namun bernama sama tetap lolos ke antarmuka pengguna.
    - **Implementasi Teknis & Solusi**:
      1. **Pembersihan Direktori Berkas Agen (`~/.browser-agent/agents/`)**:
         - Menghapus seluruh berkas numerik lama (`14.md`, `40.md`, `67.md`, `72.md`, `73.md`).
         - Menetapkan nama berkas dan ID semantik yang bersih: `arya_magang_kominfo.md`, `djadi_master_orchestrator.md`, `djadi_visual_designer.md`, `djadi_meta_ads_strategist.md`, dan `djadi_sales_closer.md`.
      2. **Pembersihan Database SQLite (`~/.browser-agent/chat_history.db`)**:
         - Menghapus baris ID numerik pada tabel `autonomous_agents` (`WHERE id IN ('14', '40', '67')`) dan menyinkronkan seluruh agen ke ID semantik unik tanpa duplikasi nama.
      3. **Pengawal Dedup Rust Host (`host/rust_host/src/main.rs`)**:
         - Memperbarui RPC `list_agents` dengan memori set `seen_ids` dan `seen_names` (case-insensitive & trimmed). Jika ada agen dengan nama yang sama, Rust Host otomatis menyaring dan hanya mengembalikan satu entitas unik.
      4. **Self-Healing & Dedup Dropdown Frontend (`extension/options.js`, `extension/sidepanel.js`)**:
         - Di `options.js` (`loadAgents`), menambahkan pencocokan normalized name dan otomatis meng-upgrade ID numerik ke semantic ID.
...
167. **Eliminasi Fatal ReferenceError `pendingVisualDocPages` & Resolusi Native RPC Pipe Starvation / SQLite Timeout (`v2.150.284`):**
    - **Akar Masalah**:
      1. `ReferenceError: pendingVisualDocPages is not defined` di `runAgentLoop` (`extension/sidepanel.js`):
         - Pada baris 7057, 7080, 7081, dan 7106, variabel `pendingVisualDocPages` digunakan untuk mengumpulkan halaman visual dokumen dan menginjeksikannya ke dalam `conversationHistory`. Namun variabel tersebut belum dideklarasikan dengan `let` dalam cakupan `runAgentLoop`, memicu exception `ReferenceError: pendingVisualDocPages is not defined` yang ditangkap blok `catch` dan menampilkan gelembung peringatan `Terjadi Kendala AI / Rate Limit`.
      2. Native RPC Pipe Starvation & Timeout SQLite:
         - Saat pengguna mengunggah dokumen PDF multi-halaman (misal: 856 KB, 20+ halaman), `save_and_parse_uploaded_file` di Host mengeksekusi `doc_parser.py --both --max-pages 20`.
         - Proses rendering gambar 20 halaman memakan waktu CPU 10-15 detik secara blocking pada single-threaded `stdin` message loop di host biner Rust (`main.rs`).
         - Sementara host sibuk merender 20 halaman, panggilan RPC lain dari ekstensi Chrome seperti `db_save_session`, `db_save_personal_memory`, dan `db_get_sessions` tertahan di antrean pipe dan akhirnya mengalami timeout 8 detik (`Error: RPC action 'db_save_session' timed out`).
      3. Akses Properti Nomor Halaman Tidak Konsisten:
         - Di loop injeksi `docAttachments` dan `runChatModeLoop`, nomor halaman diakses melalui `p.page`. Sedangkan `doc_parser.py` menghasilkan `page_num`, sehingga pada beberapa kondisi menghasilkan `Halaman undefined`.
    - **Solusi & Implementasi**:
      1. **Deklarasi dan Reset `pendingVisualDocPages`**:
         - Menambahkan deklarasi `let pendingVisualDocPages = [];` di scope `runAgentLoop` dan me-reset `pendingVisualDocPages = [];` di setiap awal iterasi per-turn sebelum pemanggilan tool.
      2. **Optimasi Max-Pages Upload Menjadi 2 Thumbnail Saja**:
         - Pada `host/rust_host/src/main.rs` dan `host/native_host.py`, parameter `--max-pages` untuk proses parsing saat upload diturunkan dari 20 menjadi 2 halaman thumbnail pratinjau.
         - Pengunggahan berkas dokumen kini selesai instan (< 300 ms), mencegah antrean pipe native messaging macet dan mengeliminasi total timeout SQLite `db_save_session` serta `db_save_personal_memory`.
         - Ketika AI agent perlu memeriksa seluruh halaman dokumen secara lengkap, agent akan memanggil tool `view_document` yang mengeksekusi konversi halaman lengkap secara on-demand.
      3. **Standarisasi Penomoran Halaman Aman**:
         - Mengubah akses properti halaman menjadi `const pNum = p.page_num || p.page || 1;` di seluruh `runAgentLoop`, visual turn injection, dan `runChatModeLoop`.
...
### 170. Rilis Versi v2.150.287 - Autonomous Semantic Critic & Quality Evaluator Engine, Anti-Overthinking Semantic Early-Exit, Auto-Fulfill Milestones, dan Targeted Refinement Protocol
- **Waktu Rilis**: 2026-09-08 23:56 WIB
- **Fokus Utama**: Menghilangkan fenomena *Overthinking & Self-Doubt Loop* (di mana agent yang sudah menjawab benar malah dipaksa berpikir lagi dan merusak jawabannya), serta membangun arsitektur *Evaluator-Optimizer / Actor-Critic* mandiri agar AI mampu mendeteksi secara objektif jika ada kekurangan pada jawabannya dan memperbaikinya secara terarah hingga 100% tuntas.
- **Akar Masalah (Root Causes)**:
  1. *Perhitungan Progres Naif Tool Count (`toolCount / 2`)*: Sistem sebelumnya menganggap setiap milestone butuh minimal 2 tool calls. Jika agent bekerja sangat efisien (cukup 1 tool call atau langsung menemukan data yang tepat), milestone intermediate tetap dianggap pending (`hasPendingMilestones = true`).
  2. *Injeksi Completion Guard Agresif*: Sistem menyuntikkan prompt bentakan internal `🛑 [SYSTEM GOAL COMPLETION GUARD]: Tugas BELUM selesai! DILARANG BERHENTI!`. Hal ini memicu fenomena psikologis AI bernama *LLM Sycophancy & Self-Doubt*, di mana model mengira jawaban sebelumnya salah/ditolak, sehingga model mulai meragukan hasil kerjanya, berhalusinasi, atau sok ngide mengubah kesimpulan yang sudah benar.
  3. *UI In-Place Content Overwrite*: Di turn berikutnya, streaming teks baru yang ngawur langsung menimpa teks jawaban benar yang sudah tampil di bubble chat pengguna.
...
- **Solusi Rekayasa Teknis Komprehensif**:
  ...
  4. **Pendaftaran Script di UI Host**:
...
### 186. Rilis Versi v2.150.303 - Robust Multi-Agent Ecosystem Synchronization, Compound Token Normalization, & Elimination of Blind Casual Companion Fallback
- **Waktu Rilis**: 2026-09-22 09:35 WIB
- **Fokus Utama**: Mengatasi kendala di mana prompt agensi/bisnis seperti *"bro enaknya djadicreative saya bikin apa lagi ya yg di todolist"* secara keliru menugaskan `Casual Companion & Personal Fact Assistant`, yang disebabkan oleh terputusnya sinkronisasi agen disk (`~/.browser-agent/agents/*.md`) di `sidepanel.js`, ketiadaan normalisasi kata majemuk (`djadicreative`), ketiadaan domain `djadi_creative` pada `goal_tracker.js`, serta adanya jebakan blind fallback ke Casual Companion.
- **Akar Masalah (Root Causes)**:
  1. *Sinkronisasi Agen di `sidepanel.js` Terputus dari Native Host*:
     - `sidepanel.js` tidak pernah memanggil RPC `list_agents` dari native host, hanya bergantung pada `chrome.storage.local.get('custom_agents')`. Akibatnya jika user membuat atau memperbarui sub-agent disk tanpa membuka `options.html`, agen baru tidak pernah termuat ke memori tab chat.
  2. *Blind Fallback ke Casual Companion di `resolveAutoAgents`*:
     - Saat kandidat agen spesialis belum termuat atau skornya 0, kode `resolveAutoAgents` baris 1065-1079 secara membabi-buta menugaskan `Casual Companion & Personal Fact Assistant` untuk seluruh prompt non-browser.
  3. *Bypass Brand Silo oleh Casual Companion*:
     - `getAgentBrand(casual_companion_agent)` mengembalikan `null`, sehingga tidak terkena aturan eliminasi `targetBrand && agentBrand && targetBrand !== agentBrand`.
  4. *Kata Majemuk Tanpa Spasi (`djadicreative`)*:
     - Frase tanpa spasi memerlukan normalisasi token awal agar cocok dengan filter regex `"djadi creative"`, `"agensi kreatif"`.
  5. *Ketiadaan Domain Djadi Creative di `extension/core/goal_tracker.js`*:
     - `inferAgentForTask` dan `extractGoalMilestones` memiliki nol blok untuk `djadi_creative`, sehingga delegasi tugas agensi selalu meleset.
  6. *Hilangnya Atribut `workers` pada Riwayat Percakapan*:
     - `sanitizeHistoryForStorage` memangkas `msg.agentInfo.workers` saat serialisasi ke SQLite, menghilangkan kartu tim agen saat sesi dibuka kembali.
- **Solusi Rekayasa Teknis Komprehensif**:
  1. **Live Agent Synchronization (`syncAgentsFromNativeHost`)**:
     - Menambahkan fungsi `syncAgentsFromNativeHost` di `extension/sidepanel.js` yang memanggil RPC native `list_agents`, `list_skills`, dan `loadPersistentMemoryFromHost`.
     - Otomatis dijalankan saat native host terhubung (`connectNativeHost`) dan saat `loadAgentsAndSkills` selesai, mengsinkronkan agen disk ke `customAgents` dan `chrome.storage.local`.
  2. **Compound Token Normalization**:
     - Menambahkan normalisasi token majemuk (`djadicreative` -> `djadi creative`, `djadiberjaya` -> `djadi berjaya`, `tiarproperty` -> `tiar property`, `cbtunesa` -> `cbt unesa`) pada `detectBrandEcosystem` dan `resolveAutoAgents`.
  3. **Strict Brand Silo Guard & Eliminasi Blind Fallback**:
     - `casual_companion_agent` didiskualifikasi secara eksplisit jika `targetBrand` aktif dan bukan domain santai (`!isCasualDomain`).
     - Pada blok fallback, jika `targetBrand` terdeteksi namun belum ada kecocokan, sistem wajib menugaskan agen utama brand tersebut (misal `djadi_master_orchestrator`), DILARANG jatuh ke Casual Companion.
     - Casual Companion hanya diizinkan jika `!targetBrand && isCasualDomain`.
  4. **Intra-Brand Djadi Scoring**:
     - Menambahkan bobot intra-brand spesifik: `djadi_master_orchestrator` (+35 untuk todo/rencana/strategi/bikin apa), `djadi_visual_designer` (+35 untuk desain/feed/deck/pitch), `djadi_meta_ads_strategist` (+35 untuk ads/cbo/cpr), `djadi_sales_closer` (+35 untuk closing/klien/retainer).
  5. **Integrasi Penuh Domain Djadi Creative pada `GoalTracker`**:
     - Memperbarui `inferAgentForTask` dan `extractGoalMilestones` pada `extension/core/goal_tracker.js` untuk memetakan sub-task Djadi ke sub-agent yang relevan.
     - Menggunakan helper kompak `makeM` sehingga line count `goal_tracker.js` tetap ramping (777 baris <= 798 baris).
  6. **Preservasi Workers pada Riwayat Percakapan**:
     - Memperbarui `sanitizeHistoryForStorage` agar menyimpan `workers: msg.agentInfo.workers.map(...)` ke basis data chat history.
  7. **Verifikasi Pengujian & Standar Sub-800 Baris**:
