...
50. **Anti-Jitter Auto-Scroll Engine & Non-Conflicting 280px Viewport Spacer Architecture:** Memperbaiki tuntas bug "jedug-jedug" (scroll jitter) dan elemen UI terbawah yang terpotong/tertutup floating input bar di Newtab. Solusi mencakup: (1) Menghapus pemanggilan `scrollIntoView` yang bentrok dengan fixed floating bar, (2) Memperbesar ruang napas bawah (`padding-bottom: 280px !important` pada `.fullscreen-chat-main` dan `360px` saat stickman aktif), (3) Menerapkan scrolling satu arah non-konflik (`window.scrollTo({ top: scrollTarget + 2000 })`), serta (4) Membatasi `MutationObserver` hanya pada penambahan node (`childList`) dan teks (`characterData`) dengan RAF throttling ketat guna mengabaikan mutasi class/style/animasi pulsa yang sebelumnya memicu jitter layar.
