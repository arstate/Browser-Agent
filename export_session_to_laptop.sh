...
mkdir -p "$TEMP_EXPORT/conversations"
...
if [ -d "$HOME/.gemini/antigravity-cli/brain/$CONV_ID" ]; then
  echo "[+] Menyalin riwayat percakapan Antigravity (Brain)..."
  cp -r "$HOME/.gemini/antigravity-cli/brain/$CONV_ID" "$TEMP_EXPORT/brain/"
fi

# 2. Copy Antigravity CLI Conversation DB
if [ -f "$HOME/.gemini/antigravity-cli/conversations/${CONV_ID}.db" ]; then
  echo "[+] Menyalin database percakapan CLI (${CONV_ID}.db)..."
  cp -r "$HOME/.gemini/antigravity-cli/conversations/${CONV_ID}"* "$TEMP_EXPORT/conversations/" 2>/dev/null || true
fi

# 3. Copy Browser Agent Local DB (Chat history SQLite)
...
# 5. Buat Script Auto-Restore untuk dijalankan di Laptop
...
if [ -d "$DIR/brain/$CONV_ID" ]; then
  ...
  echo "[✔] Riwayat Antigravity (Brain) berhasil dipasang: $HOME/.gemini/antigravity-cli/brain/$CONV_ID"
fi

# 2. Restore Antigravity Conversations DB
mkdir -p "$HOME/.gemini/antigravity-cli/conversations"
if [ -d "$DIR/conversations" ] && [ "$(ls -A "$DIR/conversations" 2>/dev/null)" ]; then
  cp -r "$DIR/conversations/"* "$HOME/.gemini/antigravity-cli/conversations/"
  echo "[✔] Database sesi CLI berhasil dipasang ke: $HOME/.gemini/antigravity-cli/conversations/"
fi

# 3. Restore Browser Agent Local DB
