...
function renderPresentationLocationCard(containerEl, options = {}) {
      ...
      if (rpcFn) {
        ...
        try {
          ...
          if (res?.status === "ok" && res.selected_file) {
            ...
            // Auto send user prompt to resume ReAct agent loop
  ...
  if (btnConfirm) {
    btnConfirm.addEventListener('click', () => {
      ...
      const chatInput = document.getElementById("chat-input");
      if (chatInput) {
        chatInput.value = `📁 Lokasi penyimpanan telah saya set ke: ${selectedPath}. Silakan lanjutkan perancangan slide deck sekarang.`;
        if (typeof handleSendMessage === "function") {
          setTimeout(() => handleSendMessage(), 100);
        } else {
          const sendBtn = document.getElementById("btn-send");
